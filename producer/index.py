...
Single-file Lambda entrypoint for AWS Console deployment.
...
def validate_payload(event: dict) -> None:
	...
	missing = [field for field in MANDATORY_FIELDS if not event.get(field)]
	if missing:
	...
	missing_data = [field for field in MANDATORY_DATA_FIELDS if not data.get(field)]
	if missing_data:
		raise ValueError(f"Missing or empty mandatory data fields: {missing_data}")


def build_kinesis_record(event: dict) -> dict:
	"""Enrich incoming event with producer metadata for Kinesis."""
	return {
...
def publish_to_kinesis(client, record: dict, partition_key: str) -> dict:
	"""Put one record to Kinesis and return the SDK response."""
...
def lambda_handler(event: dict, context) -> dict:
	...
	partition_key = str(event["CustomerId"])
